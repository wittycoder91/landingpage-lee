...
const Industries = () => (
  <section className="py-20 bg-white">
    <div className="max-[1250px] mx-auto text-center mb-12">
      <h2 className="text-4xl font-bold mb-4">Customized IT for Every Industry</h2>
      <p className="text-lg text-gray-700">We can tailor IT solutions designed to meet the unique needs of your business.</p>
    </div>
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 pl-20 pr-10">
      {industries.map((item, idx) => (
        <div key={idx} className="relative flex items-start bg-blue-50 rounded-2xl p-6 shadow-sm min-h-[152px]">
          <img src={item.icon} alt={item.title} className="absolute left-[-50px] w-24 h-w-24 mr-6" />
    ...
    <div className="flex justify-center mt-12">
      <button className="bg-blue-600 text-white px-6 py-2 rounded-full font-semibold text-lg shadow hover:bg-blue-700 transition">Let's Get Started <span className="ml-2">&gt;</span></button>
    </div>
  </section>
);
