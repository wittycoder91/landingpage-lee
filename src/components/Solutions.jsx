...
const Solutions = () => {
  return (
    <section className="w-full bg-[#f4faff] py-16 px-2 bg-no-repeat bg-position-bottom" style={{ backgroundImage: `url(${bgSolution})` }}>
      <div className='max-w-[1250px] mx-auto'>
        {/* Steps */}
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-8 justify-center mb-20 px-5">
            {steps.map((step, idx) => (
            <div key={idx} className="flex-1 bg-blue-500 rounded-2xl p-7 text-white shadow-lg flex flex-col items-center min-w-[260px] max-w-xs mx-auto">
                ...
                <p className="text-base font-normal text-blue-50 text-center">{step.desc}</p>
            </div>
            ))}
        </div>
        {/* Solutions Grid */}
        <div className="max-w-6xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold text-center mb-2">Solutions to protect your business</h2>
            <p className="text-center mb-10">We offer a wide range of solutions that can be tailored to your unique business needs.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8 p-8 mb-10">
            {solutions.map((sol, idx) => (
                <div
                key={idx}
                className={`flex items-start gap-4 ${idx < solutions.length - 2 ? 'border-b border-gray-200 pb-4' : ''}`}
                >
                <img src={sol.icon} alt={sol.title} className="w-12 h-12 rounded-full bg-white p-2 shadow object-contain" />
                <div>
                    <h3 className="font-bold text-lg mb-1">{sol.title}</h3>
                    <p className="text-gray-600 text-base leading-snug">{sol.desc}</p>
                </div>
                </div>
            ))}
            </div>
            <div className="flex justify-center">
            <button className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-8 rounded-full text-lg shadow transition">Secure My Business &gt;&gt;</button>
            </div>
        </div>
      </div>
    </section>
